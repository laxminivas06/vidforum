...
begin
  ...
  on conflict (id) do update set email = excluded.email, full_name = coalesce(nullif(excluded.full_name, ''), profiles.full_name);
  return new;
end;
$$;
...
create or replace function public.has_permission(p_institution_id uuid, p_code text)
...
create or replace function public.is_assigned_faculty(p_section_id uuid, p_subject_id uuid)
returns boolean language sql stable as $$
  select exists (
...
create policy students_select on students for select
  using (institution_id in (select public.my_institution_ids())
         and public.has_permission(institution_id, 'students.read'));
create policy students_insert on students for insert
  with check (institution_id in (select public.my_institution_ids())
              and public.has_permission(institution_id, 'students.create'));
create policy students_update on students for update
  using (institution_id in (select public.my_institution_ids())
         and public.has_permission(institution_id, 'students.update'))
  with check (institution_id in (select public.my_institution_ids()));
...
create policy attendance_sessions_select on attendance_sessions for select
  using (institution_id in (select public.my_institution_ids())
         and (public.has_permission(institution_id, 'attendance.read.all')
              or public.is_assigned_faculty(section_id, subject_id)));
create policy attendance_sessions_insert on attendance_sessions for insert
  with check (institution_id in (select public.my_institution_ids())
              and public.is_assigned_faculty(section_id, subject_id));
  ...
  using (
    exists (
      ...
      where ur.profile_id = profiles.id
        and ur.institution_id in (select public.my_institution_ids())
        and public.has_permission(ur.institution_id, 'users.role.assign')
    )
  );
  ...
  using (
    exists (
      ...
      where fp.id = face_embeddings.face_profile_id
        and fp.institution_id in (select public.my_institution_ids())
        and public.has_permission(fp.institution_id, 'ai_attendance.biometric.read')
    )
  );
