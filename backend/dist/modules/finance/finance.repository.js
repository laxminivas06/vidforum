...
class FinanceRepository {
    async findFeeRecordsByInstitution(institutionId) {
        const query = `
      SELECT 
        sf.id,
        s.id as "studentId",
    ...
    async recordPayment(invoiceId, studentId, amount, method, receivedBy, receiptNumber) {
        const res = await database_1.db.query('SELECT record_payment($1, $2, $3, $4::payment_method, $5, $6) as payment_id', [invoiceId, studentId, amount, method.toLowerCase(), receivedBy, receiptNumber]);
        return res.rows[0]?.payment_id;
    }
    async getSummary(institutionId) {
        const res = await database_1.db.query(`SELECT 
