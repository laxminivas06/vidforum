...
| **Backend Framework** | **Python 3.11+ with FastAPI** | Native asynchronous concurrency (`async`/`await`), automatic OpenAPI 3.0 generation, high-performance Pydantic v2 data serialization, and direct compatibility with Python AI/ML ecosystem. |
...
| **Asynchronous Jobs** | **Celery / ARQ Background Workers** | Asynchronous execution of heavy workloads: batch Excel marks processing, bulk SMS/voice campaign calls, document generation, and daily absentee alerts. |
...
graph TD
    ...
    subgraph Gateway["API & Security Layer"]
        ...
        APIGateway["FastAPI Core Router (/api/v1)"]
        AuthMiddleware["JWT Auth & Refresh Rotation"]
        TenantMiddleware["Tenant Isolation Middleware<br/>(institution_id Enforcer)"]
        RBACMiddleware["Two-Tier RBAC & Resource Validator<br/>(User + Role + Tenant + Resource + Action)"]
    end

    subgraph CoreServices["Backend Domain Modules (FastAPI)"]
    ...
    subgraph Persistence["Persistence & Infrastructure Layer"]
        ...
        RedisCache[(Redis 7<br/>Sessions, Rates, Queue Broker)]
        ObjectStorage[(S3 / MinIO<br/>Encrypted Documents & Photos)]
        BackgroundWorkers["Async Celery Workers<br/>(Batch Jobs & Notifications)"]
    end
...
1. **Multi-Tenant Scoping:**
   ...
   - All SQLAlchemy database queries are automatically wrapped with `.filter(Model.institution_id == request.state.institution_id)` unless the user is a verified Super Admin accessing platform-level entities.
