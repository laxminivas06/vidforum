...
exports.institutionRepository = exports.InstitutionRepository = void 0;
...
class InstitutionRepository {
    ...
    `;
        const res = await database_1.db.query(query);
        return res.rows;
    }
    async findByIdOrCode(idOrCode) {
        const res = await database_1.db.query(`SELECT i.*, p.code as plan_code, p.name as plan_name 
       FROM institutions i 
       LEFT JOIN subscription_plans p ON p.id = i.plan_id 
       WHERE i.id = $1 OR i.code = $1`, [idOrCode]);
        return res.rows[0] || null;
    }
    async findModules(institutionId) {
        const res = await database_1.db.query('SELECT module_code, is_enabled, config FROM module_configurations WHERE institution_id = $1', [institutionId]);
        return res.rows;
    }
    async getStats(institutionId) {
        const [studentsRes, staffRes, attendanceRes, feesRes] = await Promise.all([
            database_1.db.query("SELECT count(*) FROM students WHERE institution_id = $1 AND status = 'active'", [institutionId]),
            database_1.db.query("SELECT count(*) FROM staff WHERE institution_id = $1 AND employment_status = 'active'", [institutionId]),
            database_1.db.query(`
        SELECT 
    ...
    async upsertModule(institutionId, moduleCode, isEnabled) {
        const res = await database_1.db.query(`INSERT INTO module_configurations (institution_id, module_code, is_enabled)
       VALUES ($1, $2, $3)
       ON CONFLICT (institution_id, module_code) 
       DO UPDATE SET is_enabled = EXCLUDED.is_enabled, updated_at = now()
       RETURNING *`, [institutionId, moduleCode, isEnabled]);
        return res.rows[0];
    }
}
exports.InstitutionRepository = InstitutionRepository;
exports.institutionRepository = new InstitutionRepository();
