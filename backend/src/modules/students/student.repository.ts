...
export class StudentRepository {
  async findStudents(institutionId: string, filters: { search?: string; classId?: string; sectionId?: string; status?: string }) {
    let query = `
      SELECT 
        s.id,
  ...
  async findStudentMasterById(id: string, institutionId: string) {
    ...
    const feeRes = await db.query(
      `SELECT sf.total_amount, sf.balance_due,
       ...
       GROUP BY sf.id, sf.total_amount, sf.balance_due`,
      [student.id]
    );
