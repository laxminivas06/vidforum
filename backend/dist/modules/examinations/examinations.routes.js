...
router.get('/exams', tenant_middleware_1.tenantMiddleware, async (req, res) => {
    try {
    ...
    `;
        const result = await database_1.db.query(query, [instId]);
...
router.get('/marks', tenant_middleware_1.tenantMiddleware, async (req, res) => {
    try {
        const instId = req.institutionId;
...
router.post('/import-excel', tenant_middleware_1.tenantMiddleware, async (req, res) => {
    try {
        const { examId, subjectId, rows } = req.body;
        // rows: Array of { admissionNumber, marks }
        if (!Array.isArray(rows) || rows.length === 0) {
            (0, api_response_1.sendError)(res, 'No import rows provided', 400);
            return;
        }
        ...
        for (const row of rows) {
            const studentRes = await database_1.db.query('SELECT id, first_name, last_name FROM students WHERE admission_number = $1 AND institution_id = $2', [row.admissionNumber, req.institutionId]);
